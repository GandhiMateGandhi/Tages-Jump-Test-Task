<template>
  <div class="card-wrapper">
    <div class="card-list">
      <div
          class="card-item"
          :class="{'card-item_discount-label': item.price.old_price}"
          v-for="item in allItems"
          :key="item.id">

        <img class="card-item__image" :src="item.image.src" :alt="item.name"/>

        <div class="card-item__info-section">
          <span class="card-item__code">{{ item.code }}</span>

          <p class="card-item__name">{{ item.name }}</p>

          <div class="card-item__price-section">
          <span v-if="item.price.old_price" class="card-item__old-price">
            {{ Math.floor(item.price.old_price) }}&#8381;
          </span>
            <span class="card-item__current-price">
            {{ Math.floor(item.price.current_price) }}&#8381;
          </span>
          </div>

          <div class="card-item__icon-section">
            <span v-if="item.isAddedToCart" @click="addToCart(item.id)">
             <svg class="card-item__favorites-icon svg-inline--fa fa-check-circle fa-w-16" aria-hidden="true"
                  focusable="false" data-prefix="far" data-icon="check-circle"
            ...
            <span v-else @click="addToCart(item.id)">
            ...
            <span v-if="item.isFavorite" @click="addToFavorite(item.id)">
              <svg class="card-item__favorites-icon svg-inline--fa fa-heart fa-w-16" aria-hidden="true"
            ...
            <span v-else @click="addToFavorite(item.id)">
              <svg class="card-item__favorites-icon svg-inline--fa fa-heart fa-w-16"
...
export default {
  name: "Cards",
  methods: {
    ...mapActions(['setLocalStorage', 'addToFavorite', 'addToCart']),
  },
  computed: {
    ...mapGetters(['allItems']),
  },
